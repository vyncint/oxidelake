...
- Pinned host memory: with `cuda` active and a driver present, allocate page-locked memory through cudarc (the `cuMemHostAlloc` path) for true DMA without staging copies; otherwise fall back transparently to `AlignedBuf`. The path taken is observable (`MemoryClass::{Pinned, Pageable}`) so tests and the TUI can report it. In 0.x this is an available allocation, not the operator path: `Gpu*Exec` uploads from pageable Arrow buffers and downloads into pageable `Vec`s, so the staging copy is still paid. Pinned staging for operator transfers is P6 (#26).
...
- **IO abstraction is `object_store::ObjectStore`**, the trait DataFusion reads through. Default `object_store::local::LocalFileSystem`. `io-uring` feature (Linux target-gated): `UringLocalFileSystem` implementing `ObjectStore` for `file://` — `get`, ranged `get_opts`/`get_ranges`, `head`, `put`, `list`, `delete` — on a dedicated ring thread with completions bridged to `oneshot` futures. A shared conformance test runs against both implementations.

  **Implemented and conformance-tested; not used by sessions in 0.x.** Nothing constructs `UringLocalFileSystem`: every session registers the default `LocalFileSystem`, so enabling the feature changes nothing at run time (#24). Registering it into the session's `RuntimeEnv` is the remaining step, and it is deliberately not taken yet — the 2026-09-02 audit measured the ring *slower* than `LocalFileSystem` on this workload, so wiring it would be a regression dressed as a feature. It waits on a measurement that says otherwise.
